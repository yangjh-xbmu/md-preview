...
- [x] T001 [P] Run `go test ./...` against `main_test.go` and `update_test.go` from the repository root and record the baseline result.
- [x] T002 [P] Run `npm --prefix frontend run build` for `frontend/src/App.tsx` and record the baseline TypeScript and Vite result.
- [x] T003 [P] Verify the existing `Quit`, `WindowFullscreen`, `WindowUnfullscreen`, and `WindowIsFullscreen` exports in `frontend/wailsjs/runtime/runtime.d.ts` and `frontend/wailsjs/runtime/runtime.js` before changing application code.
...
- [x] T004 [US1] Add a failing zero-dependency TypeScript test in `frontend/tests/keyboard.test.ts` covering unmodified `F11`, `Option+F11`, and rejected extra modifiers by importing the planned predicates from `frontend/src/keyboard.ts`.
- [x] T005 [US1] Implement the tested full-screen shortcut predicate in `frontend/src/keyboard.ts` so `F11` and `altKey + F11` are accepted while unrelated modifier combinations are rejected.
...
- [x] T006 [US2] Extend `frontend/tests/keyboard.test.ts` with a failing exact `Ctrl+Q` predicate test covering case normalization and rejected `Cmd+Q`, `Ctrl+Alt+Q`, and `Ctrl+Shift+Q` combinations.
- [x] T007 [US2] Implement the tested quit shortcut predicate in `frontend/src/keyboard.ts` and wire both predicates into `frontend/src/App.tsx`, importing the existing `Quit` runtime function and returning before the general modifier dispatch.
- [x] T008 [US2] Update the full-screen menu hint in `frontend/src/App.tsx` to show `F11 / ⌥F11` while keeping the existing menu action and `fullscreen` state label unchanged.
- [x] T009 [US2] Update the shortcut table in `README.md` with `F11 / Option+F11` for full screen and `Ctrl+Q` for quitting, without changing the existing shortcut meanings.
...
- [x] T010 [P] Update the keyboard handling note in `CLAUDE.md` to record the macOS full-screen and quit shortcut behavior and the relevant verification commands.
- [x] T011 [P] Run `go test ./...` against `main_test.go` and `update_test.go` after implementation.
- [x] T012 [P] Run `npm --prefix frontend run build` for `frontend/src/App.tsx` after implementation.
- [x] T013 [P] Run `wails build` using `wails.json` and confirm the desktop package completes.
- [x] T014 Run the macOS manual smoke test in `specs/006-mac-fullscreen-quit-shortcut/quickstart.md`, including preview focus, menu focus, full-screen restoration, and process exit checks.
