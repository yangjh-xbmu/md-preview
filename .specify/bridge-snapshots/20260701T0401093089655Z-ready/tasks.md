...
- [X] T001 Verify current GitHub release asset naming and compatible Windows asset shape for `md-preview-v<VERSION>-windows-amd64.zip`
- [X] T002 Add application version injection plan to `.github/workflows/release.yml` and local Wails build command expectations
- [X] T003 [P] Confirm frontend Wails binding files that must be updated for new backend methods in `frontend/wailsjs/go/main/App.*`
...
- [X] T004 Add update data types, default constants, and app version variable in `update.go` and `main.go`
- [X] T005 Add persistent update settings load/save helpers in `update.go`
- [X] T006 Add update status state storage and event emission helpers in `update.go`
- [X] T007 [P] Add settings default and persistence tests in `update_test.go`
- [X] T008 [P] Add semantic version comparison and platform asset selection tests in `update_test.go`
...
- [X] T009 [P] [US1] Add latest release metadata tests in `update_test.go`
- [X] T010 [P] [US1] Add download, digest verification, and extraction tests in `update_test.go`
- [X] T011 [US1] Add startup check non-blocking behavior test in `update_test.go`

### Implementation for User Story 1

- [X] T012 [US1] Implement release metadata fetch and stable release filtering in `update.go`
- [X] T013 [US1] Implement compatible asset download, SHA-256 verification, and Windows zip extraction in `update.go`
- [X] T014 [US1] Implement startup update check wiring in `app.go`
- [X] T015 [US1] Implement staged Windows install handoff and safe unsupported fallback in `update.go`
...
- [X] T016 [P] [US2] Add backend toggle and manual-check-while-disabled tests in `update_test.go`

### Implementation for User Story 2

- [X] T017 [US2] Add Wails-bound update settings and manual check methods in `app.go`
- [X] T018 [US2] Update generated frontend bindings in `frontend/wailsjs/go/main/App.js` and `frontend/wailsjs/go/main/App.d.ts`
- [X] T019 [US2] Add update toggle and manual check controls to `frontend/src/App.tsx`
- [X] T020 [US2] Add compact update menu styling in `frontend/src/App.css`
...
- [X] T021 [P] [US3] Add backend status transition tests in `update_test.go`
- [X] T022 [US3] Add frontend build validation coverage through `npm --prefix frontend run build`

### Implementation for User Story 3

- [X] T023 [US3] Subscribe to `update-status-changed` and render update status in `frontend/src/App.tsx`
- [X] T024 [US3] Add install/restart action for ready updates in `frontend/src/App.tsx` and backend binding in `app.go`
...
- [X] T025 [P] Update user-facing automatic update documentation in `README.md`
- [X] T026 Update release workflow build command in `.github/workflows/release.yml` to inject `main.appVersion`
- [X] T027 Run `go test ./...`
- [X] T028 Run `npm --prefix frontend run build`
- [X] T029 Run `wails build -ldflags "-X main.appVersion=0.1.1"` and restore unrelated generated side effects if any
- [X] T030 Run quickstart scenarios from `specs/004-auto-update/quickstart.md`
