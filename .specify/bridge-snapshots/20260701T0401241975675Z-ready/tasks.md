...
- [ ] T001 Run `go test ./...` and confirm all existing tests pass before changes
- [ ] T002 Run `npm --prefix frontend install` and `npm --prefix frontend run build` to confirm the frontend builds cleanly
...
- [ ] T003 [P] [US1] Add Go unit tests in `main_test.go` covering: ASCII target (`README`), Chinese target (`Git基本概念与常用命令`), space-encoded target (`My%20Note`), `.html` suffix target, `.md` suffix target, missing target, and invalid extension target

### Implementation for User Story 1

- [ ] T004 [US1] Update `ResolveWikiLink` in `app.go` to decode percent-encoded `href` values with `net/url.PathUnescape` before extension normalization and file resolution
- [ ] T005 [US1] Verify the frontend click handler in `frontend/src/App.tsx` still correctly skips external/anchor/mailto links and shows the existing "Wiki link target not found" message for unresolved targets
...
- [ ] T006 [P] Run `go test ./...` and confirm all tests pass after the change
- [ ] T007 [P] Run `npm --prefix frontend run build` and confirm the frontend build succeeds after the change
- [ ] T008 [P] Optionally update `README.md` or `CLAUDE.md` if user-facing behavior or developer notes need clarification
