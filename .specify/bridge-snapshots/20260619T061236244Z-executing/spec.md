...
2. **Given** the common icon source and Windows icon asset have been regenerated, **When** the app is built, **Then** the Wails build completes and packages the updated icon asset.
...
- **FR-002**: The common app icon source MUST be updated from the supplied artwork so clean builds do not fall back to old artwork.
- **FR-003**: The Windows icon asset MUST remain a valid `.ico` file with multiple sizes suitable for desktop display.
- **FR-004**: The app build MUST continue to complete successfully after the icon is replaced.
- **FR-005**: The change MUST NOT alter Markdown rendering behavior, command-line behavior, preview UI behavior, or application metadata unrelated to the icon.
...
- Wails uses the existing common icon source and Windows icon asset locations already present in the repository.
